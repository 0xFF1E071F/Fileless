...
static unsigned long resolveAddress(const char* symName) {
	struct kprobe probe = {
		.symbol_name = symName,
	};
	...
	pr_info("Address of %s found at: 0x%px", symName, probe.addr);
...
{
    hook->address = resolveAddress(hook->name);
...
int fh_install_hook(struct ftrace_hook *hook)
{
	...
	pr_alert("Successfully hooked %s", hook->name);
    return 0;
}
